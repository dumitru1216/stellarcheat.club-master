...
#define vfunc(index, func, sig) auto func { return reinterpret_cast<sig>((*(uint32_t**)this)[index]) member_func_args
	...
	public:
		...
		{
			using original_fn = void(__thiscall*)(SDK::IMaterial*, int, bool);
		...
		void set_vec_value(float r, float g, float b) {
			using original_fn = void(__thiscall*)(SDK::IMaterial*, float, float, float);
		...
		void modulate(const CColor color)
		{
			ColorModulate(color.RGBA[0] / 255.f, color.RGBA[1] / 255.f, color.RGBA[2] / 255.f);
			AlphaModulate(color.RGBA[3] / 255.f);
		}
