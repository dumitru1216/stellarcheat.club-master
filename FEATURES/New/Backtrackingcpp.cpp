...
std::vector<Backtracking_Record> CBacktracking::GetPriorityRecords(SDK::CBaseEntity* entity)
{
	...
	auto local_player = INTERFACES::ClientEntityList->GetClientEntity(INTERFACES::Engine->GetLocalPlayer());
	if (!local_player || local_player->GetHealth() <= 0 || player_records[player_index].size() <= 0 || !entity->IsAlive())
	...
	for (const auto& record : player_records[player_index])
	{
		...
		if(resolver.GetDeltaTime(record.resolve_record.on_shot_eye_curtime) < 0.2)
			resolved_records.push_back(record);


		if (resolver.IsResolved(record.resolve_record.resolve_type))
			resolved_records.push_back(record);
	}
...
void CBacktracking::InvalidateBoneCache(SDK::CBaseEntity* entity)
{
	...
	static uintptr_t g_iModelBoneCounter = **(uintptr_t**)(InvalidateBoneCache + 10); //	Offsets::InvalidateBoneCache = FindPatternIDA("client_panorama.dll", "80 3D ? ? ? ? 00 74 16 A1");
