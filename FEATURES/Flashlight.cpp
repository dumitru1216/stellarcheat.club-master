...
CFlashLightEffect* CFlashLight::CreateFlashLight(int nGetIndex, const char *pszTextureName, float flFov, float flFarZ, float flLinearAtten)
{
	...
	CFlashLightEffect *pFlashLight = new CFlashLightEffect;
...
void CFlashLight::UpdateFlashLight(CFlashLightEffect *pFlashLight, const Vector &vecPos, const Vector &vecForward, const Vector &vecRight, const Vector &vecUp)
{
	...
	oUpdateLight(pFlashLight, pFlashLight->m_nGetIndex, vecPos, vecForward, vecRight, vecUp, pFlashLight->m_flFov, pFlashLight->m_flFarZ, pFlashLight->m_flLinearAtten, pFlashLight->m_bCastsShadows, pFlashLight->m_szTextureName);
}
...
void CFlashLight::RunFrame()
{
	...
	/*if (UTILS::INPUT::input_handler.GetKeyState(UTILS::INPUT::input_handler.keyBindings(SETTINGS::settings.desync_switch)) & 1)
	{
	...
	*/
	if (pFlashLight)
	{
		...
		pFlashLight->m_flFov = fabsf(13 + 37 * cosf(x += 0.002f));
