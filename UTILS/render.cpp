...
namespace RENDER
{
	...
	void Circle3DFilled(Vector position, float points, float radius, CColor color)
	{
		float step = (float)M_PI * 2.0f / points;
		...
		for (float a = 0; a < (M_PI * 2.0f); a += step)
		{
			...
			vertices.push_back(SDK::Vertex_t(Vector(radius * cosf(a) + position.x, radius * sinf(a) + position.y, position.z)));
