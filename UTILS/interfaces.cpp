...
namespace INTERFACES
{
	...
	void InitInterfaces()
	{
		...
		clientstate = reinterpret_cast<SDK::CClientState*>(UTILS::FindSignature("engine.dll", "A1 ? ? ? ? 8B 80 ? ? ? ? C3") + 1);
		...
		m_dwFireBullets = UTILS::FindSignature("client_panorama.dll", "55 8B EC 51 53 56 8B F1 BB ? ? ? ? B8") + 0x131;
	...
	void* FindInterface(const char* Module, const char* InterfaceName)
	{
		...
		if (!Interface)
			std::cout  << InterfaceName << red <<" not found!\n" << std::endl;
		else
			std::cout << InterfaceName << green <<" 0x" << Interface << "\n "<< std::endl;
