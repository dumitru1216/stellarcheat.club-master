...
void Hitmarker::Paint() {
	...
	for (pIter = pHitmarkers.begin(); pIter != pHitmarkers.end(); ) {
		...
		CColor pColor = CColor(255,255,255,255);
		CColor Color = CColor(255, 255, 255, 150);

		int pLineSize = awsize;
		...
		/////
		if (SETTINGS::settings.player_hitmarker_combo == 0) {
			RENDER::DrawLine(pos2D.x - pLineSize / 4, pos2D.y - pLineSize / 4, pos2D.x - (pLineSize), pos2D.y - (pLineSize), pColor);
			RENDER::DrawLine(pos2D.x - pLineSize / 4, pos2D.y + pLineSize / 4, pos2D.x - (pLineSize), pos2D.y + (pLineSize), pColor);
			RENDER::DrawLine(pos2D.x + pLineSize / 4, pos2D.y + pLineSize / 4, pos2D.x + (pLineSize), pos2D.y + (pLineSize), pColor);
			RENDER::DrawLine(pos2D.x + pLineSize / 4, pos2D.y - pLineSize / 4, pos2D.x + (pLineSize), pos2D.y - (pLineSize), pColor);
		}
		if (SETTINGS::settings.player_hitmarker_combo == 1)
		{
			RENDER::DrawCircle(pos2D.x, pos2D.y, 1, 100, Color);
			RENDER::DrawCircle(pos2D.x, pos2D.y, 5, 100, Color);
		}
		/*else if(pc == 0) {
			RENDER::DrawF((pos2D.x), pos2D.y - 15, FONTS::visuals_headshot_font, true, true, CColor(0, 150, 0, 255), "");
		} */
...
void Hitmarker::PlayerHurt(SDK::IGameEvent* pEvent) {
	...
	pInfo.dmg_health = pEvent->GetInt(("dmg_health"));
